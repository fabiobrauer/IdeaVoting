...
function DoneBeforeButton({ postId }) {
    ...
    const fetchPost = async () => {
        try {
            const response = await axios.get(`http://localhost:3000/posts/${postId}/doneBefore`);
    ...
    const toggleDoneBefore = async () => {
        ...
        try {
            ...
            if (doneBefore) {
                ...
                response = await axios.delete(`http://localhost:3000/posts/${postId}/doneBefore`, {
                    headers: {
                ...
                response = await axios.patch(`http://localhost:3000/posts/${postId}/doneBefore`, {}, {
                    headers: {
                        Authorization: `Bearer ${localStorage.getItem('token')}`
                    }
                });
